//! Client library: the view layer, shared by every client binary.
//!
//! Binaries in this crate:
//! - `brickrail-client` — combined client + simulation in one process.
//! - `client-only` — client alone, to be paired with `brickrail-server`.
...
/// The client view: layout state in the main world, a camera, and gizmo rendering.
/// Knows nothing about where the layout came from or who simulates it.
...
/// Spawns a layout straight into the main world, with no command queue and no
/// simulation behind it. The client-only binary uses this to have something to draw
/// until it can receive a layout from a server.
pub struct LocalLayoutPlugin(pub Layout);

impl Plugin for LocalLayoutPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(LocalLayout(self.0.clone()));
        app.add_systems(Startup, spawn_local_layout);
    }
}

#[derive(Resource)]
struct LocalLayout(Layout);

fn spawn_local_layout(
    layout: Res<LocalLayout>,
...
/// Client-side simulation: the layout SubApp plus the command plumbing that feeds
/// it. Used by the combined binary, which runs client and simulation in one process.
/// The client-only binary omits this and will talk to a server over the network
/// instead, once that transport exists.
...
/// Demo scenario: spawns a test layout and drives a train across it. Stands in for
/// loading a real layout and for user interaction, neither of which exists yet.
/// Requires [`ClientSimulationPlugin`], since it issues simulation commands.
pub struct DemoScenarioPlugin;

impl Plugin for DemoScenarioPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, queue_init_commands);
    }
}
